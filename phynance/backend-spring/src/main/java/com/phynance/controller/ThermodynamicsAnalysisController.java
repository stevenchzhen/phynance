...
public class ThermodynamicsAnalysisController {
    ...
    public ResponseEntity<?> analyze(@RequestBody ThermodynamicsAnalysisRequest request) {
        ...
        List<MarketDataDto> mainOhlcv = financialDataService.getHistoricalData(request.getSymbol(), request.getStartDate(), request.getEndDate());
        ...
        List<List<MarketDataDto>> relatedOhlcv = new ArrayList<>();
        if (request.getRelatedSymbols() != null) {
            for (String rel : request.getRelatedSymbols()) {
                List<MarketDataDto> relData = financialDataService.getHistoricalData(rel, request.getStartDate(), request.getEndDate());
